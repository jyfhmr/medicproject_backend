...
@Injectable()
export class DebitNoteSalesService {
    constructor(
        private usersService: UsersService,
        private correlativeService: CorrelativeService,
        @InjectRepository(DebitNoteSale)
        private debitNoteSaleRepository: Repository<DebitNoteSale>,

        @InjectRepository(config_admistrative_reason)
        private reasonRepository: Repository<config_admistrative_reason>,
    ) {}

    async create(CreateCreditNotSaleDto: CreateDebitNoteSaleDto, userId: number) {
        // Busca al usuario
        ...
        const newCreditNote = this.debitNoteSaleRepository.create({
            ...otherFields,
        ...
        await this.debitNoteSaleRepository.save(newCreditNote);

        return 'This action adds a new Credit Note';
    }

    async findAll(query: any): Promise<{ totalRows: number; data: DebitNoteSale[] }> {
        ...
        try {
            const [resCount, resData] = await Promise.all([
                this.debitNoteSaleRepository.count({ where }),
                query?.export
                    ? this.debitNoteSaleRepository.find({
                          relations,
                          where,
                          order: { id: order },
                      })
    ...
    async findOne(id: number): Promise<DebitNoteSale> {
        const relations = {
            user: true,
            userUpdate: true,
            typeMoney: true,
        ...
        return await this.debitNoteSaleRepository.findOne({ where: { id: id }, relations });
    }

    async update(id: number, updateCreditNotSaleDto: UpdateDebitNoteSaleDto, userId: number) {
        ...
        const creditNote = await this.debitNoteSaleRepository.findOne({
            where: { id: id },
        ...
        await this.debitNoteSaleRepository.save(creditNote);
    ...
    async changeStatus(id: number): Promise<string | Error> {
        const updateStatus = await this.debitNoteSaleRepository.findOneBy({ id });
        updateStatus.isActive = !updateStatus.isActive;

        try {
            await this.debitNoteSaleRepository.save(updateStatus);
    ...
    async processExcel(buffer: Buffer): Promise<any> {
        ...
        const entities = jsonData.map((data: any) => {
            return this.debitNoteSaleRepository.create({
        ...
        await this.debitNoteSaleRepository.save(entities);

        return { message: 'Datos importados correctamente', total: entities.length };
    }

    async listMoney(): Promise<DebitNoteSale[]> {
        return await this.debitNoteSaleRepository.find({ where: { isActive: true } });
    }
