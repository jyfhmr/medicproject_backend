...
@Injectable()
export class CreditNotSalesService {
    constructor(
        private usersService: UsersService,
        private correlativeService: CorrelativeService,
        @InjectRepository(CreditNotSale)
        private creditNoteSalesRepository: Repository<CreditNotSale>,

        @InjectRepository(config_admistrative_reason)
        private reasonRepository: Repository<config_admistrative_reason>,
    ) {}

    async create(createCreditNotSaleDto: CreateCreditNotSaleDto, userId: number) {
        // Busca al usuario
        const user = await this.usersService.findOne(userId);

        // Desestructura 'motive' y otros campos
        const { motive, ...otherFields } = createCreditNotSaleDto;
        ...
        const newCreditNote = this.creditNoteSalesRepository.create({
            ...otherFields,
        ...
        await this.creditNoteSalesRepository.save(newCreditNote);

        return 'This action adds a new Credit Note';
    }
    async findAll(query: any): Promise<{ totalRows: number; data: CreditNotSale[] }> {
        ...
        try {
            const [resCount, resData] = await Promise.all([
                this.creditNoteSalesRepository.count({ where }),
                query?.export
                    ? this.creditNoteSalesRepository.find({
                          relations,
                          where,
                          order: { id: order },
                      })
    ...
    async findOne(id: number): Promise<CreditNotSale> {
        const relations = {
            user: true,
            userUpdate: true,
            typeMoney: true,
        ...
        return await this.creditNoteSalesRepository.findOne({ where: { id: id }, relations });
    }

    async update(id: number, updateCreditNotSaleDto: UpdateCreditNotSaleDto, userId: number) {
        // Busca el usuario que realizó la actualización
        const user = await this.usersService.findOne(userId);
        console.log(updateCreditNotSaleDto);

        // Intenta encontrar la nota de crédito para asegurarte de que existe antes de actualizar
        const creditNote = await this.creditNoteSalesRepository.findOne({
            where: { id: id },
        ...
        const { motive, ...otherFields } = updateCreditNotSaleDto;
        ...
        Object.assign(creditNote, otherFields); // Actualiza otros campos con los datos de updateCreditNotSaleDto
        ...
        await this.creditNoteSalesRepository.save(creditNote);
    ...
    async changeStatus(id: number): Promise<string | Error> {
        const updateStatus = await this.creditNoteSalesRepository.findOneBy({ id });
        updateStatus.isActive = !updateStatus.isActive;

        try {
            await this.creditNoteSalesRepository.save(updateStatus);
    ...
    async processExcel(buffer: Buffer): Promise<any> {
        ...
        const entities = jsonData.map((data: any) => {
            return this.creditNoteSalesRepository.create({
        ...
        await this.creditNoteSalesRepository.save(entities);

        return { message: 'Datos importados correctamente', total: entities.length };
    }

    async listMoney(): Promise<CreditNotSale[]> {
        return await this.creditNoteSalesRepository.find({ where: { isActive: true } });
    }
